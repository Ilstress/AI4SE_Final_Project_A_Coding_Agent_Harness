"""SPEC §3.2: DeepSeekAdapter — LLM adapter for the DeepSeek API.
...
class DeepSeekAdapter(AbstractLLM):
    """LLM adapter for the DeepSeek API (OpenAI-compatible format).
    ...
    def __init__(
        ...
        self._model = model
        self._api_base = api_base or _DEEPSEEK_API_BASE
    ...
    async def call(self, messages: list[dict]) -> LLMResponse:
        """Send messages to DeepSeek and return a standardized response.
            ...
            try:
                ...
                if response.status_code in _RETRYABLE_STATUS:
                    if attempt < _MAX_ATTEMPTS - 1:
                        delay = 2**attempt
                        logger.debug(
                            "DeepSeek API returned %d, retrying in %ds (attempt %d/%d)",
                        ...
                        continue
                    raise LLMFatalError(
                        f"Max retries exceeded for DeepSeek API. "
                ...
                if response.status_code in _FATAL_STATUS:
                    body = response.text
                    raise LLMFatalError(
                        f"Non-retryable error from DeepSeek API: "
                ...
                if attempt < _MAX_ATTEMPTS - 1:
                    delay = 2**attempt
                    logger.debug(
                        "Network error for DeepSeek API, retrying in %ds: %s",
                        delay,
                        e,
                    )
                    await asyncio.sleep(delay)
                    ...
                    raise LLMFatalError(
                        f"Max retries exceeded for DeepSeek API after network error: {e}"
                    ) from e

        # Should be unreachable — safety net
        raise LLMFatalError(
            f"Max retries exceeded for DeepSeek API: {last_error}"
        )
